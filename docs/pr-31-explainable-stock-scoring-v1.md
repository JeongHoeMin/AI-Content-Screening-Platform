...
- direction별 score weight를 소유하는 immutable `ScoringPolicyConfig`와 exhaustive
  `DirectionScoreCatalog`를 정의한다.
- `CompanyImpact` 하나의 direction 해석을 보존하는 immutable `ScoreContribution` Domain을 추가한다.
- `CompanyScore`에 evidence와 같은 순서·길이의 contribution breakdown을 추가하고, score가 그
  contribution 합과 일치하도록 validator로 고정한다.
- 새 deterministic `EvidenceAwareScoringStrategy`가 contribution을 만들고 합산하며,
  `DefaultScoringEngine`은 Strategy 결과를 조립만 하도록 유지한다.
- Bootstrap은 explicit scoring catalog/config를 주입한다.
...
Catalog는 duplicate direction 또는 missing direction이 있으면 생성·bootstrap 시 fail-fast한다.
...
`ScoreContribution`은 하나의 원본 `CompanyImpact`와 그 Catalog 해석을 보존하는 immutable Pydantic
값이다.
...
- v1에서 `value`는 Catalog `weight`와 같다. count, recency, source, uncertainty multiplier를 추가하지
  않는다.
...
`CompanyScore`는 기존 `company`, `score`, `evidences` 외에 `contributions` tuple을 보관한다.

- `company`는 input `CompanyEvidence.company`와 동일 객체다.
- `evidences`는 input `CompanyEvidence.impacts`와 같은 객체·순서로 보존한다.
- `contributions` 길이는 `evidences` 길이와 같고 같은 순서다.
- 각 `contributions[index].impact is evidences[index]`가 반드시 성립한다.
- `score`는 contribution `value`의 정확한 합이다. float 계산은 `math.fsum`을 사용해 결정적인 합산
  기준을 하나로 고정한다.

이 불변식은 `CompanyScore` Domain validator에서 fail-fast한다. 기존 consumer가 필요로 하는
`CompanyScore.score`와 `evidences` 계약은 유지한다.

### 5. Policy와 configuration 경계

- `ScoringPolicyConfig`는 weight 범위, Catalog 및 policy version을 Pydantic으로 검증하는 immutable
  configuration 값이다.
- score weight와 factor/reason mapping은 Catalog가 소유한다. Strategy는 mapping을 hard-code하지 않고
  주입된 Catalog를 조회한다.
...
- v1 config는 방향 weight의 부호를 강제하지 않는다. 정책적 positive/negative 의미는 Catalog entry와
  test로 명시하며, 모든 값은 finite float이어야 한다.
...
EvidenceAggregation
    ↓
EvidenceAwareScoringStrategy(catalog)
...
- `EvidenceAwareScoringStrategy`는 network, LLM, Prompt, Directory, database, cache를 호출하지
  않는다.
...
- Default Catalog의 weight가 현재 rule policy와 같으므로 기존 score 값과 recommendation boundary
  결과는 변하지 않아야 한다.
- `CompanyScore`의 새 `contributions`는 score provenance를 보강하는 additive Domain 확장이다.
...
- non-finite weight (`NaN`, positive/negative infinity)를 거부하는지 검증한다.
- `ScoreContribution`의 impact identity, `CompanyScore` evidence/contribution 길이·순서·identity 및
  score-sum 불변식을 검증한다.
...
- default Catalog에서 기존 `RuleScoringStrategy`의 score와 새 strategy의 score가 동일한지 검증한다.
...
- `DOMAIN_MODEL.md`: ScoreContribution, DirectionScoreCatalog, CompanyScore breakdown 계약을 추가한다.
